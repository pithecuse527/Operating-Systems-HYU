...
int Execution(char** args, int mode, int background, int symbol_loc);  // fork by mode
...
int main(void)
{
  ...
  while(shell_is_alive)
  {
    ...
    *(usr_cmd+cmd_len-1) = '\0';    // swap the '\n' to null character
    ...
    if(Execution(args, mode, background, special_symbol_loc) == 0) continue;    // fork followed by mode
    else
    {
  ...
  else if(pid == 0) // child
  {
    ...
    if(mode == PIPE_MODE)
    {
      ...
      else if(pid2 == 0) // child again
      {
        ...
        close(fdl[WRITE_END]);
        execvp(*(args+symbol_loc+1), args+symbol_loc+1);  // execute the right half of args (after the pipe symbol)
...
int MyParse(char* usr_cmd, char** args, int* background, int* mode)
{
  ...
  *background = FALSE;
  ...
  while(*(args+i))  // choose the mode from here (REDIRECTION_MODE or NORMAL_MODE)
  {
    // printf("!%s!\n", *(args+i));  // testing what tokens are
    ...
    if(strcmp(*(args+i), "<") == 0) // we found there is a file redirection (input)
    {
